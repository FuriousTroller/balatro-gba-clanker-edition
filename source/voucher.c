...
const VoucherInfo* get_voucher_registry_entry(u8 id) 
{
    for (int i = 0; i < VOUCHER_REGISTRY_SIZE; i++) {
        if (voucher_registry[i].id == id) return &voucher_registry[i];
    }
    return NULL;
}
...
{
    if (_current_shop_voucher == NULL) return;
...
{
    if (_current_shop_voucher == NULL) return;
    if (_current_shop_voucher->sprite_object != NULL) return; // Already spawned
    ...
    memcpy32(&tile_mem[4][VOUCHER_TID], &voucher_gfx0Tiles[_current_shop_voucher->info->id * 128], 128);
